...
const weeklyStats = computed(() => [
  { value: String(documentStore.documents.length), label: '文档总数', trend: '—', icon: 'book' },
  { value: String(annotationStore.annotations.length), label: '批注总数', trend: '—', icon: 'bookmark' },
  { value: String(noteStore.notes.length), label: '笔记总数', trend: '—', icon: 'pen-tool' },
  { value: '—', label: '本周阅读时长', trend: '—', icon: 'clock' },
])
...
const readingList = computed(() =>
  documentStore.documents
    ...
    .map((d) => ({
      title: d.title,
      meta: `${d.file_type.toUpperCase()} · 阅读至${d.read_progress}%`,
      progress: `${d.read_progress}%`,
      id: d.id,
    }))
)
      ...
      <div class="main-inner">
        ...
        <div class="topbar">
          ...
          <div class="topbar__actions">
            <button class="upload-btn" @click="router.push('/documents')">
        ...
        <!-- Weekly Overview -->
        <div v-else class="weekly-overview">
          ...
          <div class="weekly-overview__grid">
            <div v-for="stat in weeklyStats" :key="stat.label" class="wo-item">
              ...
              <span v-if="stat.trend !== '—'" class="wo-item__trend up">{{ stat.trend }}</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="readingList.length > 0" class="timeline-wrap">
          <div class="timeline">
            <div class="timeline__header">
              <h2>继续阅读</h2>
              <a href="#" @click.prevent="router.push('/documents')">查看全部 →</a>
            </div>
            <div class="tl-list">
              <div v-for="item in readingList" :key="item.id" class="tl-item" @click="router.push(`/reader/${item.id}`)">
          ...
          <!-- Recent Activity -->
          <div v-if="recentActivity.length > 0" class="timeline-divider"></div>
              ...
              <div v-for="(item, idx) in recentActivity" :key="idx" class="tl-item">
                ...
                <div class="tl-item__body">
                  ...
                  <div class="tl-item__meta">
                    <span :class="['tl-tag', item.type==='highlight' ? 'tag-hl' : 'tag-note']">{{ item.type==='highlight' ? '批注' : '笔记' }}</span>
                    {{ item.doc }}
                  </div>
                </div>
...
.main-inner { width: 100%; max-width: 1600px; }
...
/* Weekly Overview */
.weekly-overview { background: var(--bg-card); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 1.5rem 2rem; margin-bottom: 1.5rem; }
.weekly-overview__header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.25rem; }
.weekly-overview__header h2 { font-family: var(--font-display); font-size: 1.1rem; font-weight: 500; color: var(--text-primary); }
.weekly-overview__grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; }
.wo-item { display: flex; align-items: center; gap: 1rem; padding: 0.5rem 0.75rem; border-radius: 8px; transition: background 0.15s; }
.wo-item:hover { background: var(--accent-light); }
.wo-item__icon { width: 44px; height: 44px; border-radius: 10px; background: var(--accent-light); display: flex; align-items: center; justify-content: center; color: var(--accent); flex-shrink: 0; }
.wo-item__icon svg { width: 22px; height: 22px; }
.wo-item__info { flex: 1; min-width: 0; }
.wo-item__value { font-family: var(--font-display); font-size: 1.5rem; font-weight: 600; color: var(--text-primary); line-height: 1.2; }
.wo-item__label { font-family: var(--font-ui); font-size: 0.7rem; color: var(--text-muted); margin-top: 0.1rem; }
.wo-item__trend { font-family: var(--font-ui); font-size: 0.65rem; padding: 0.15rem 0.4rem; border-radius: 10px; white-space: nowrap; }
.wo-item__trend.up { background: #D1FAE5; color: #065F46; }

/* Timeline */
.timeline-wrap { display: flex; gap: 0; margin-bottom: 2rem; }
.timeline-wrap .timeline { flex: 1; margin-bottom: 0; min-width: 0; max-height: 420px; display: flex; flex-direction: column; }
.timeline-wrap .timeline .tl-list { flex: 1; overflow-y: auto; min-height: 0; }
.timeline-wrap .timeline:first-child { padding-right: 1.5rem; }
.timeline-wrap .timeline:last-child { padding-left: 1.5rem; }
.timeline-divider { width: 1px; flex-shrink: 0; background: var(--border-color); margin: 0.5rem 0; }
.timeline__header { display: flex; align-items: baseline; gap: 0.6rem; margin-bottom: 0.6rem; padding-bottom: 0.4rem; border-bottom: 1px solid var(--border-color); }
.timeline__header h2 { font-family: var(--font-display); font-size: 1rem; font-weight: 500; color: var(--text-primary); }
.timeline__header a { font-family: var(--font-ui); font-size: 0.75rem; color: var(--text-muted); text-decoration: none; margin-left: auto; }
.timeline__header a:hover { color: var(--accent); }
.tl-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.65rem 0.6rem; border-radius: 6px; transition: background 0.12s; cursor: pointer; }
.tl-item:hover { background: var(--accent-light); }
.tl-item + .tl-item { border-top: 1px solid var(--border-color); }
.tl-item__icon { width: 34px; height: 34px; border-radius: 8px; background: var(--accent-light); display: flex; align-items: center; justify-content: center; color: var(--accent); flex-shrink: 0; }
.tl-item__icon svg { width: 17px; height: 17px; }
.tl-item__body { flex: 1; min-width: 0; }
.tl-item__title { font-size: 0.88rem; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.tl-item__meta { font-family: var(--font-ui); font-size: 0.7rem; color: var(--text-muted); margin-top: 0.1rem; }
.tl-item__right { font-family: var(--font-ui); font-size: 0.7rem; color: var(--text-muted); flex-shrink: 0; }
.tl-item__progress { font-family: var(--font-ui); font-size: 0.7rem; color: var(--accent); flex-shrink: 0; }
.tl-tag { padding: 0.1rem 0.35rem; border-radius: 4px; font-size: 0.65rem; font-weight: 600; }
.tag-hl { background: #FEF3C7; color: #92400E; }
.tag-note { background: #DBEAFE; color: #1E40AF; }
.tl-quote { color: var(--text-secondary); font-style: italic; font-size: 0.82rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
